...
Схема opencode требует `limit.context` и `limit.output` вместе. OpenCodex выводит блок только когда
оба значения подтверждены точными metadata provider или явной настройкой custom model. Если одно
из значений неизвестно, весь блок опускается; старый placeholder `32000` больше не используется.

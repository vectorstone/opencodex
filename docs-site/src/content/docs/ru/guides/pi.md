...
`contextWindow` и `maxTokens` — независимые подтверждённые возможности. Каждое поле выводится
только при наличии его значения; для отсутствующего значения Pi использует собственный default.
`maxTokens` берётся из точных metadata provider или явной настройки custom model и ограничивается
context window, когда известны оба значения. Старый placeholder `32000` больше не используется.

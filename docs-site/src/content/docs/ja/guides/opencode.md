...
opencode のスキーマでは `limit.context` と `limit.output` を組にする必要があります。OpenCodex は両方が権威ある値として判明している場合だけブロックを出力します。output は正確な provider メタデータ、または custom model の明示設定から取得されます。どちらかが不明ならブロック全体を省略し、旧来の `32000` プレースホルダーは使用しません。

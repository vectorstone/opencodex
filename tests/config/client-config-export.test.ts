...
describe("split config-export public facade", () => {
  ...
  test("preserves each moved format's serialized fields, order and owned fragment", () => {
    const context = ctx({ models: [{
      ...
      maxOutputTokens: 8192, inputModalities: ["text", "image"], reasoningEfforts: ["none", "high"],
    }] });
    const cases = [
      ...
      ["zcode", ["provider", "opencodex"], '{"provider":{"opencodex":{"name":"OpenCodex","kind":"openai","enabled":true,"source":"custom","options":{"apiKey":"opencodex-loopback","baseURL":"http://127.0.0.1:10100/v1","apiKeyRequired":true},"models":{"test/known":{"name":"known (test)","modalities":{"input":["text","image"],"output":["text"]},"limit":{"context":8192,"output":8192},"reasoning":{"enabled":true,"variants":["high"]}}}}}}'],
    ] as const;
...
describe("hub-resolved Fast exports", () => {
  const eligible: ExportModel = {
    ...
    fastRowAvailable: true, contextWindow: 8192, maxOutputTokens: 8192, inputModalities: ["text", "image"],
...
`);
  ...
  test("pi bytes include session affinity and authoritative output limits, to the last newline", () => {

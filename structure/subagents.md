...
### Routed V2 collaboration plaintext delivery (fork F-006)

The fork also carries an always-on contract for routed providers that upstream's opt-in compiler does
not cover. `src/responses/namespace-tool-compat.ts` removes the `message.encrypted` marker from the
flattened declarations of `collaboration.spawn_agent`, `collaboration.send_message`, and
`collaboration.followup_task` before they reach a third-party Responses provider, and attaches
`encrypted_function_args: []` when an authorized routed call is restored to one of those three names.
Codex 0.151+ reads that exact marker as plaintext delivery, so child and peer `agent_message` items
carry `input_text` instead of ChatGPT-backend ciphertext the routed provider cannot read.

The marker is scoped to request-authorized function aliases and those three message-bearing
collaboration calls. Encrypted schema markers on other tools stay untouched, the delivery marker is
never added to other collaboration tools or custom calls, and non-empty `encrypted_function_args` is

...
| Native integrations | `GET /api/native-integrations` reports live persisted intent. `PUT /api/native-integrations/codex` accepts either legacy `{ enabled: boolean }` or `{ mode: "full" | "catalog-only" | "off" }`, never both. Full owns routing/catalog/cache/compatibility artifacts; catalog-only refreshes catalog/cache and preserves provider routing and history; off restores native Codex. `src/server/management/native-integration-routes.ts` owns this validation and response contract. |

  ...
  const customModels = (config.customModels ?? []).map(cm => {
    ...
    const customMaxOutputTokens = rawProvider
      ? routedMaxOutputTokens(cm.provider, rawProvider, {
        ...
        // Fork F-004: an explicit custom-model output cap is authoritative capability metadata,
        // so it takes part in the same precedence as generated and discovered metadata.
  ...
  for (const provider of providerNames) {
    ...
    for (const meta of listModelMetadata(jawcodeProvider)) {
      ...
      const model: CatalogModel = {
        ...
        // Fork F-004: only a safe-integer capability may be published; a non-integer numeric id
        // would advertise an output maximum the client cannot represent.

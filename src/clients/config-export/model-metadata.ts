 ...
 *
 * Fork F-004. `undefined` means the catalog never declared a real maximum, and every serializer
 * must treat that as "unknown" rather than substituting a uniform budget: the wire defaults
 * (`defaultMaxOutputTokens` / `modelMaxOutputTokens`) are request defaults, not catalog capability,
 * and publishing one as a per-model maximum would tell the client something we do not know.
 */
export function authoritativeMaxOutputTokens(

...
export interface OpencodeCatalogModel {
  ...
  /**
   * Authoritative output-token capability (fork F-004), carried verbatim from `/api/models`.
   * Absent means unknown — a serializer must omit the client's limit field rather than
   * substitute a uniform budget, which would advertise a capability the model may not have.
   */
...
export interface ExportModel {
  ...
  displayName?: string;
  contextWindow?: number;
  /** Authoritative output-token capability (fork F-004); absent means unknown, never guessed. */
  maxOutputTokens?: number;

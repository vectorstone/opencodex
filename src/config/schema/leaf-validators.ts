...
export const clientIntegrationsSchema = z.object({
  // Fork F-001: `codex` is a durable three-mode switch. A missing value or
  // `true` reads as `full`, `"catalog-only"` refreshes only the catalog/cache,
  // and `false` is `off`. The boolean arm is retained for the legacy toggle
  // contract, so an existing `true`/`false` file keeps its exact meaning.

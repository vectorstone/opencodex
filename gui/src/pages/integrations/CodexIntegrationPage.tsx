...
export default function CodexIntegrationPage({
  ...
  const resource = useDataSurface<NativeStatus>(
    ...
    [apiBase],
    fetchStatus,
    { isEmpty: () => false, enabled: active, sessionCacheKey: "ocx.integrations.codex-mode.v2:" + apiBase },
  );
  ...
  return (
    <section className="integration-native-page codex-integration-page" aria-labelledby="codex-integration-title">
      ...
      {!status && resource.state.kind !== "failed-cold" && <p className="page-sub">{t("common.loading")}</p>}
      {resource.state.showError && <Notice tone="err">{t(status ? "integrations.error.stale" : "integrations.error.load")}</Notice>}
      {mutationFailed && <Notice tone="err">{t("integrations.error.change")}</Notice>}
